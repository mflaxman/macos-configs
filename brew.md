...
### Regular
```bash
$ brew reinstall ag aspell automake chromium colordiff csvkit diff-pdf glow git-delta go htop ipython jless jq libffi libusb libsecp256k1 macvim maestral node ollama openssl@3 pandoc pkg-config postgresql pyenv qrencode ruby rbenv sqlite tcl-tk tor tree vim --no-quarantine
```
Notes:
